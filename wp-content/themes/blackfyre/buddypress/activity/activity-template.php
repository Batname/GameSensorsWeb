...
function bp_activity_avatar( $args = '' ) {
	...
	function bp_get_activity_avatar( $args = '' ) {
		...
		$alt_default = !empty( $dn_default ) ? sprintf( esc_html__( 'Profile picture of %s', 'buddypress' ), $dn_default ) : esc_html__( 'Profile picture', 'buddypress' );
	...
	function bp_get_activity_secondary_avatar( $args = '' ) {
		...
		switch ( $activities_template->activity->component ) {
			case 'groups' :
				...
				if ( empty( $alt ) ) {
					$alt = esc_html__( 'Group logo', 'buddypress' );

					if ( ! empty( $name ) ) {
						$alt = sprintf( esc_html__( 'Group logo of %s', 'buddypress' ), $name );
					}
				}
			...
			case 'blogs' :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( esc_html__( 'Profile picture of the author of the site %s', 'buddypress' ), get_blog_option( $item_id, 'blogname' ) );
				}
			...
			case 'friends' :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( esc_html__( 'Profile picture of %s', 'buddypress' ), bp_core_get_user_displayname( $activities_template->activity->secondary_item_id ) );
				}
			...
			default :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( esc_html__( 'Profile picture of %s', 'buddypress' ), $activities_template->activity->display_name );
				}
...
function bp_activity_comment_count() {
	...
	function bp_activity_get_comment_count( $deprecated = null ) {
		...
		if ( ! empty( $deprecated ) ) {
			_deprecated_argument( __FUNCTION__, '1.2', sprintf( esc_html__( '%1$s no longer accepts arguments. See the inline documentation at %2$s for more details.', 'buddypress' ), __FUNCTION__, __FILE__ ) );
		}
...
function bp_activity_delete_link() {
	...
	function bp_get_activity_delete_link() {
		...
		$link = '<a href="' . esc_url( $url ) . '" class="button item-button bp-secondary-action ' . $class . ' confirm" rel="nofollow">' . esc_html__( 'Delete', 'buddypress' ) . '</a>';
...
function bp_send_public_message_button( $args = '' ) {
	...
	function bp_get_send_public_message_button( $args = '' ) {
		...
		$r = bp_parse_args( $args, array(
			...
			'link_title'        => esc_html__( 'Send a public message on your activity stream.', 'buddypress' ),
			'link_text'         => esc_html__( 'Public Message', 'buddypress' ),
